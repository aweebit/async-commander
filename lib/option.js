...
function handleRejection(err) {
  if (err?.code === 'commander.invalidArgument') {
    const command = /** @type {Command} */ (this.__recommander_command);
    const name = this.attributeName();
    const value = command.__recommander_rawOptionValues[name];
    const source = command.__recommander_rawOptionValueSources[name];
